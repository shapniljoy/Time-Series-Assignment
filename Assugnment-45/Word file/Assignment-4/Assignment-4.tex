...
First we have to check for the number of lags in each variable. Using the vector autoregression \itshape varsoc command, we can identify the optimal number of lags for each variable.
The optimal lag is 1. So we run an ADF test with \itshape lags(1).
...
For the Random walk process we see that ADF test correctly identifies the unit root process almost $95\%$ times which aligns with the $95\%$ confidence interval at rejection rate is $5\%$.
